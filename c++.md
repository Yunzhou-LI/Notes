...
​	常在单参数的构造函数前添加explicit关键字，禁止隐式转换
...
bar(10);  // ✅ int型整数 10 会隐式转换成 Foo(10)
```
...
在进行一些插入或删除操作或扩容时，都会有可能导致迭代器失效，进而非法访问。那么该如何做呢？？

- 若进行erase或remove操作，通常都会返回一个新的迭代器，可通过新的迭代器来进行更新。
- 尽量使用安全稳定的容器接口如splice。
- 对于map，若要删除，可以先记录要删除的迭代器，记录其进行自增后的迭代器，再删除当前迭代器。

### 18. 服务器框架安全性校验方式

CRC循环冗余校验码：检错不纠错,不能防止恶意篡改。

HMAC-SHA256：既可以做完整性校验，又可以防止恶意篡改，计算复杂性更高，但安全性更高。

### 19 C++之子类构造时显示初始化父类

在子类继承父类之后，构造子类时，若不显示初始化父类，其实编译器会默认调用父类的默认构造函数（无参构造），为了正确初始化子类以及父类中的成员，应该显示调用父类的构造函数，避免基类无默认构造时，子类忘记初始化父类时会报错，以及若希望指定父类的某个有参构造时，更应该显式调用父类的构造函数。
...
如果函数后面标了 `noexcept`，编译器会认为：

- **函数调用期间绝不会抛出异常**；
- 如果它真抛出异常，**程序会被直接终止（`std::terminate()`）**。

### 21.erase配合remove惯用写法删除元素
...
```c++
cpp
auto new_end = std::remove(endpoints_.begin(), endpoints_.end(), endpoint);
```

✅ **做了什么：**

- 遍历整个容器
- 把 **所有不等于 `endpoint` 的值**前移，覆盖掉要移除的值
- 返回一个迭代器 `new_end`：
  - 指向**新逻辑末尾**
  - **但容器大小没有变**，后面多余的值还留着
...
```c++
cpp
endpoints_.erase(new_end, endpoints_.end());
```

✅ **做了什么：**

- 删除 `[new_end, endpoints_.end())` 这个区间的元素
- 真正修改 `endpoints_` 的 size

📖 **状态变化：**

```
arduinoCopyEdit[1, 3, 4, 5]
容器 size = 4
```

适用容器vector，string，deque

### 22 assert()

在调试阶段，assert(expression)用于检查哪些绝不该发生错误的地方，若表达式为false，会立即终止程序，在发布模式中，编译器会自动忽略assert()
